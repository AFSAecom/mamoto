  ...
  return (
    <main className="p-6">
      ...
      <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3">
        {rows.map(m => (
          <Link key={m.id} href={`/motos/${m.id}`} className="border rounded overflow-hidden hover:shadow">
            <div className="aspect-[4/3] bg-black/10 relative">
              {m.display_image
                ? <Image src={m.display_image} alt={`${m.brand||''} ${m.model||''}`} fill sizes="(max-width:768px) 100vw, 33vw" className="object-cover" />
