...
export default function MotoSpecsPage() {
  ...
  const loadMoto = async () => {
    const { data } = await supabase.from('motos').select('id, brand, model').eq('id', motoId).single();
    if (data) setMoto(data as Moto);
  };
  ...
  return (
    <main className="p-6 space-y-8">
      ...
      <section className="rounded-2xl border bg-white">
        {(() => {
          const groups = new Map<string, { category: string; subcategory: string; items: Spec[] }>();
          for (const s of specs) {
            const cat = s.category ?? '';
            const sub = s.subcategory ?? '';
            const key = `${cat}__${sub}`;
            if (!groups.has(key)) groups.set(key, { category: cat, subcategory: sub, items: [] });
            groups.get(key)!.items.push(s);
          }
          const arr = Array.from(groups.values());
          if (arr.length === 0) return <div className="p-4 text-sm text-gray-600">Aucune caractéristique pour le moment.</div>;
          return (
                            ...
                            <tr key={row.id} className="border-b align-top">
                              <td className="p-2">
                                <input className="border rounded px-2 py-1 w-full"
                                  ...
                                  onChange={e=>setEditing(prev=>({...prev, [row.id]: {...prev[row.id], key_name: e.target.value}}))}
                                />
                                <div className="grid grid-cols-2 gap-2 mt-1">
                                  <input className="border rounded px-2 py-1" placeholder="Catégorie"
                                    defaultValue={row.category ?? ''}
                                    onChange={e=>setEditing(prev=>({...prev, [row.id]: {...prev[row.id], category: e.target.value}}))}
                                  />
                                  <input className="border rounded px-2 py-1" placeholder="Sous-catégorie"
                                    defaultValue={row.subcategory ?? ''}
                                    onChange={e=>setEditing(prev=>({...prev, [row.id]: {...prev[row.id], subcategory: e.target.value}}))}
                                  />
                                </div>
                              </td>
                              <td className="p-2">
                                <textarea className="border rounded px-2 py-1 w-full" rows={2}
                                  ...
                                  onChange={e=>setEditing(prev=>({...prev, [row.id]: {...prev[row.id], value_text: e.target.value}}))}
                                />
                              </td>
                              <td className="p-2">
                                <input className="border rounded px-2 py-1 w-28"
                                  defaultValue={row.unit ?? ''}
                                  onChange={e=>setEditing(prev=>({...prev, [row.id]: {...prev[row.id], unit: e.target.value}}))}
                                />
                              </td>
                              <td className="p-2">
                                <input className="border rounded px-2 py-1 w-20" type="number"
                                  defaultValue={row.sort_order ?? 0}
                                  onChange={e=>setEditing(prev=>({...prev, [row.id]: {...prev[row.id], sort_order: e.target.value ? Number(e.target.value) : 0}}))}
                                />
                              </td>
